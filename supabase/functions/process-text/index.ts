...
**⚠️ חובה לקרוא! כללים קריטיים לאחוזי נכות ותאריכים:**
...
**כלל 2: תאריכים (מתאריך / עד תאריך / מידת הנכות)**
→ **חפש טבלה עם "נכות משוקלל" או "שקלול נכות"**
→ **זוהי הטבלה הסופית! קח את התאריכים רק משם!**
→ **התעלם מתאריכים בטבלאות של ליקויים ספציפיים!**
→ בטבלה זו תמצא את "מתאריך" ו"עד תאריך" הנכונים
→ שים לב: אם כתוב "זמני" → מידת הנכות היא "זמני"
→ שים לב: אם כתוב "צמית" → מידת הנכות היא "צמית"
...
- **ת.ז:**: 
  • **סדר חיפוש (לפי עדיפות):**
    1. **בטבלת "פרטים אישיים"** תחת "מספר זהות:" - זה המקום המדויק ביותר!
    2. בשורה "זהות, מספר: XXXXXXXX" בכותרת
    3. לאחר "מספר זהות:" או "זהות:" בכל מקום במסמך
  • **זה תמיד מספר של 8-9 ספרות בלבד**
  • דוגמאות נכונות: "29393154", "11327061", "123456789"
  • **לא לקחת:** מספר ועדה (כמו "NV-60947-3000"), מספר תיק, טלפון
...
- **תאריך פגיעה**: 
  • **חפש בטבלה "פרטי הפגיעה"** תחת "תאריך הפגיעה:"
  • **או** חפש בכל מקום במסמך "תאריך האירוע:" או "תאריך פגיעה:"
  • דוגמה: "01/01/2020"
  • **רלוונטי רק לאיבה ונכות מעבודה** - החזר null אם לא קיים

**4. משתתפי הועדה - עמודות נפרדות:**
...
- **קריטי: אל תכלול "מזכיר הישיבה" או "מזכיר ישיבה"!**
- **החזר רק את חברי הועדה הרפואיים** (רופאים, מומחים)
- **החזר בעמודות נפרדות:**
  • "משתתף ועדה 1": "שם (תפקיד)"
  • "משתתף ועדה 2": "שם (תפקיד)"
  • "משתתף ועדה 3": "שם (תפקיד)"
  • "משתתף ועדה 4": "שם (תפקיד)"
- **דוגמה:**
  • "משתתף ועדה 1": "מיכאל בלומנטל (כירורגיה אורולוגית)"
  • "משתתף ועדה 2": "דוד כהן (רופא פנימי)"
- **תרגם את התפקידים לעברית תקנית** אם הם מופיעים בסדר הפוך
- **אם יש פחות מ-4 משתתפים**: השאר את השדות הריקים כ-null
...
- **אבחנה**: התיאור הרפואי של המצב הרפואי
  • דוגמה: "תכיפות במתן שתן", "כאבי גב", "דיכאון", "אוטם שריר הלב, הפרעת הסתגלות"
  • זה התיאור המילולי של המחלה/בעיה הרפואית
  • חפש בטבלת "אבחנות" או בעמודה "אבחנה"
  • **אם יש כמה אבחנות - הפרד אותן בפסיק**
  • דוגמה לטבלת אבחנות:
    - "410 - אוטם שריר הלב"
    - "F43.2 - הפרעת ההסתגלות"
    → החזר: "אוטם שריר הלב, הפרעת הסתגלות"
  
- **סעיף ליקוי**: הקוד או הסעיף המשפטי/רפואי - **כל הסעיפים ביחד!**
  • דוגמה: "410, F43.2" או "34א)ב(, 5א" או "(II)(23)2()א"
  • זה הקוד הפורמלי לפי הטבלאות הרפואיות
  • **חובה לחלץ את כל סעיפי הליקוי מהטבלה - גם מספרים וגם אותיות!**
  • דוגמה: אם בטבלה יש:
    - שורה 1: "410" - אוטם שריר הלב
    - שורה 2: "F43.2" - הפרעת ההסתגלות
    → החזר: "410, F43.2"
  • **הפרד ביניהם עם פסיק ורווח**
  
- **אחוז הנכות**: האחוז הכולל המשוקלל הסופי
  • **קריטי! סדר חיפוש (חובה לעקוב אחרי הסדר!):**
    1. **משפט סיכום סופי - זו העדיפות המוחלטת הגבוהה ביותר!**
       - **תמיד** חפש במסמך משפט שמכיל את המילים:
         • "נקבעה לך נכות קבועה של"
         • "נקבעה לך נכות זמנית של"
         • "נקבעה לך נכות של"
         • "ואילך נקבעה לך נכות"
       - דוגמאות:
         • "מ- 26/03/2025 ואילך נקבעה לך נכות קבועה של 84.08%" → **84.08%**
         • "נקבעה לך נכות צמיתה של 75.5%" → **75.5%**
       - **אם מצאת משפט כזה - השתמש רק בו! התעלם מכל טבלה אחרת!**
       - **זהו האחוז האמיתי והסופי והמדויק ביותר במסמך!**
    
    2. **טבלת "החלטה אחרונה מתאריך" או "החלטה רפואית מתאריך"**:
       - חפש כותרת כמו "החלטה אחרונה מתאריך XX/XX/XXXX"
       - הטבלה תכיל: "נכות רפואית", "מתאריך", "עד תאריך"
       - דוגמה:
         | עד תאריך | מתאריך | נכות רפואית |
         | 31/10/2025 | 16/02/2025 | 21.0% |
       - **קח מעמודה "נכות רפואית"** → החזר: "21%"
    
    3. **טבלה סופית** עם "נכות", "שקלול נכות", "נכות משוקלל":
       - דוגמה:
         | נכות | שקלול נכות | נכות משוקלל |
         | 30.7% | 31% | זמני |
       - **קח מעמודה "נכות"** → החזר: "30.7%"
    
    4. **טבלת החלטות רגילה** - "אחוז נכות" בטבלה עם ליקויים
  
  • **אל תוסיף** "צמית" או "זמני" לאחוז - אלה שייכים ל"מידת הנכות"
  
- **אחוז הנכות משוקלל**: השקלול לחישוב פטור ממס
  • **קריטי! סדר חיפוש (חובה לעקוב אחרי הסדר!):**
    1. **משפט סיכום סופי - זו העדיפות המוחלטת הגבוהה ביותר!**
       - **תמיד** חפש במסמך משפט שמכיל את המילים:
         • "נקבעה לך נכות קבועה של"
       ...
       - דוגמאות:
         ...
         • "נקבעה לך נכות צמיתה של 75.5%" → **75.5%**
       - **אם מצאת משפט כזה - השתמש רק בו! התעלם מכל טבלה אחרת!**
       - **זהו האחוז המשוקלל האמיתי והסופי והמדויק ביותר במסמך!**
    
    2. **טבלה עם "שקלול נכות"** - אם קיימת טבלה סופית עם עמודה זו:
       | נכות | שקלול נכות | נכות משוקלל |
       | 30.7% | 31% | זמני |
       → החזר: "31%"
    
    3. **אחרת** - החזר את אותו ערך כמו "אחוז הנכות" (אם אין עמודה נפרדת)
  
  • **לא תמיד יש שדה נפרד**, במקרים רבים זה זהה ל"אחוז הנכות"
  
- **מתאריך ועד תאריך - קריטי!**
  • **סדר חיפוש (בעדיפות גבוהה מאוד!):**
    1. **משפט סיכום סופי** - **זו העדיפות הגבוהה ביותר!**:
       - חפש במסמך משפט כמו:
         • "מ-XX/XX/XXXX ואילך נקבעה לך נכות קבועה של X%"
         • "מ-XX/XX/XXXX עד XX/XX/XXXX נקבעה לך נכות של X%"
         • "מ-XX/XX/XXXX נקבעה לך נכות של X%"
       - דוגמה: "מ- 26/03/2025 ואילך נקבעה לך נכות קבועה של 84.08%"
       - **מתאריך**: "26/03/2025"
...
- **מידת הנכות**: האם הנכות זמנית או צמיתה
  • **כיצד לקבוע (בעדיפות!):**
    1. **משפט סיכום סופי** - **זו העדיפות הגבוהה ביותר!**:
       - אם כתוב "נקבעה לך נכות קבועה" → "צמית"
       - אם כתוב "נקבעה לך נכות זמנית" → "זמני"
       - אם כתוב "ואילך" במשפט → "צמית"
       - דוגמה: "מ- 26/03/2025 ואילך נקבעה לך נכות קבועה של 84.08%" → "צמית"
    
    2. **אם יש תאריך סיום ספציפי** (כמו "31/10/2025") → "זמני"
    
    3. **אם כתוב "ואילך"** או "עד להודעה חדשה" בטבלה → "צמית"
    
    4. **אם יש עמודה ספציפית** "נכות משוקלל" או "מידת הנכות" עם הערך - קח אותו
  
  • דוגמאות:
    - משפט: "נקבעה לך נכות קבועה של 84.08%" → מידת הנכות: "צמית"
    - עד תאריך: 31/10/2025 → מידת הנכות: "זמני"
    - עד תאריך: עד להודעה חדשה → מידת הנכות: "צמית"

...
**הוראות חשובות:**
1. התעלם מעמוד הראשון אם הוא מכיל רק מכתב החלטה כללי בלבד
2. התמקד בדפים שמכילים פרטים מפורטים של הועדה הרפואית ובטבלאות
3. עבור שם הטופס - זהה את הסוג המדוייק לפי תוכן המסמך
4. חפש בטבלאות מפורטות ובחלקים הרלוונטיים של המסמך
...
**פרטי ועדה (חפש בכותרות ובתחילת דפים):**
- **סוג ועדה**: "ועדת אשכול נפגעי עבודה", "ועדה רפואית לנכות כללית" וכדומה
- **שם טופס**: זהה במדוייק:
  • אם יש תאריך פגיעה ומדובר בעבודה: "נכות מעבודה"
  • אם מדובר בנכות כללית: "נכות כללית"  
  • אם מדובר באיבה: "איבה"
  • אם מדובר בשארים: "שארים"
- **סניף הוועדה**: "סניף ראשי חדרה", "סניף תל אביב" וכדומה

**פרטי מבוטח (בראש המסמך או בטבלאות):**
- **שם המבוטח**: שם המבוטח המלא (כמו "מקס כהן")
- **ת.ז:**: מספר תעודת זהות בן 8-9 ספרות (כמו "23685886")
- **תאריך פגיעה**: תאריך הפגיעה המקורית (רק בתיקי נכות מעבודה/איבה)

**משתתפי הועדה (חפש בקטע נפרד):**
- **משתתפי הועדה**: רופאים ומזכירי ועדה (כמו "ד״ר בר-נוף אלברטו", "עזרא בן משיטה חנה")

**פרטי אבחנה ותקופה (חפש בטבלאות החלטה):**
- **תקופה**: תקופות הנכות (כמו "18/01/2022 עד 31/08/2023")
- **אבחנה**: כל האבחנות (כמו "אוטם שריר הלב, הפרעת הסתגלות") - הפרד בפסיק
- **סעיף ליקוי**: קודי סעיפים (כמו "410, F43.2")

**פרטי נכות (בטבלאות החלטה):**
- **אחוז הנכות הנובע מהפגיעה**: האחוזים המקוריים (כמו "20%, 15%, 10%")
- **הערות**: הערות הועדה (כמו "הוועדה החליטה שאין להגדיל את אחוזי הנכות על פי תקנה 15")
- **מתאריך**: תאריך תחילת כל תקופה
- **עד תאריך**: תאריך סיום (אם זמני)

**פרטי שקלול (בטבלת שקלול בסוף):**
- **מידת הנכות**: "זמני" או "צמית" לכל תקופה
- **אחוז הנכות משוקלל**: האחוזים המשוקללים (כמו "40%, 35%, 28%")
- **שקלול לפטור ממס**: אחוזי הפטור ממס (כמו "36%, 32%, 28%")

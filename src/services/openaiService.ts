...
**כללים קריטיים:**
1. התעלם לחלוטין מהעמוד הראשון אם הוא מכיל רק מכתב החלטה כללי
2. חפש רק בעמודים הפנימיים שמכילים טבלאות מפורטות וממצאים רפואיים
3. כל שדה חייב להתבסס על מידע מדויק מהמסמך - לא להמציא כלום!
4. אם המידע לא נמצא במסמך - החזר null במקום לנחש

**טקסט המסמך:**
${text}

**משימה:** חלץ מידע רק מהעמודים הרלוונטיים (לא העמוד הראשון):

**1. סוג הוועדה והטופס:**
- **סוג ועדה**: חפש "ועדת אשכול נפגעי עבודה" או "ועדה רפואית" בכותרות
- **שם טופס**: קבע לפי תוכן:
  • אם יש "תאריך פגיעה" + "מעבודה" = "נכות מעבודה"
  • אם אין תאריך פגיעה = "נכות כללית"
  • אם מוזכר "איבה" = "איבה"
- **סניף הוועדה**: "סניף ראשי חדרה" או דומה בתחתית המסמך

**2. פרטי המבוטח (מהטבלאות הפנימיות):**
- **שם המבוטח**: מהשדה "שם:" בטבלת פרטים אישיים
- **ת.ז:**: מספר זהות 8-9 ספרות מהטבלאות
- **תאריך פגיעה**: תאריך מדויק מהשדה "תאריך פגיעה" (רק אם קיים)

**3. משתתפי הועדה (מקטע "משתתפי הוועדה"):**
- חפש רופאים ומזכירים (כמו "ד״ר בר-נוף אלברטו קרדיולוגיה")

**4. אבחנות ונכות (מטבלאות החלטה):**
- **תקופה**: כל התקופות (כמו "18/01/2022 עד 31/08/2023, 01/09/2023 עד 31/03/2024")
- **אבחנה**: כל האבחנות מהטבלאות - הפרד בפסיק (כמו "אוטם שריר הלב, הפרעת הסתגלות")
- **סעיף ליקוי**: קודים (כמו "410, F43.2")
- **אחוז הנכות הנובע מהפגיעה**: כל האחוזים בפסיק (כמו "20%, 15%, 10%")
- **הערות**: הערות הועדה מהטבלאות
- **מתאריך**: תאריך התחלה הראשון
- **עד תאריך**: תאריך הסיום הזמני (אם קיים)

**5. שקלול נכות (מטבלת שקלול בסוף):**
- **מידת הנכות**: זמני/צמית לכל תקופה בפסיק (כמו "זמני, זמני, צמית")
- **אחוז הנכות משוקלל**: אחוזים משוקללים בפסיק (כמו "40%, 35%, 28%")
- **שקלול לפטור ממס**: אחוזי פטור ממס בפסיק (כמו "36%, 32%, 28%")

**דוגמאות לזיהוי מהמסמך הנוכחי:**
- שם: "מקס כהן" (מטבלת פרטים אישיים)
- ת.ז: "23685886" (מטבלת פרטים אישיים)
- תאריך פגיעה: "18/10/2021" (מהכותרת)
- אבחנות: "אוטם שריר הלב, הפרעת הסתגלות" (מטבלת אבחנות)
- משתתפי ועדה: "ד״ר בר-נוף אלברטו קרדיולוגיה, עזרא בן משיטה חנה"
